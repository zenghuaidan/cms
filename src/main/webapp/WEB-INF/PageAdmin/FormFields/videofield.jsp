@model System.Xml.XmlNode
@{
    Layout = null;    
    System.Xml.XmlNode dataw = (System.Xml.XmlNode)ViewData["dataw"];
    System.Xml.XmlNode wdef = (System.Xml.XmlNode)ViewData["wdef"];
    Dictionary<string, string> fpm = CmsWebCore.Cms.LogicCms.getFieldCommon(Model,wdef);
    string videoId = (dataw == null) ? fpm["fdefval"] : CmsWebCore.Common.Helper.getFieldRaw(dataw, fpm["fname"]);    
    
    CmsWebCore.Models.CmsWebEntities db = new CmsWebCore.Models.CmsWebEntities();
    CmsWebCore.Models.CmsPage pg = db.CmsPage.Where(p => p.template.Equals("Video")).FirstOrDefault();
    CmsWebCore.Common.CmsWebContent _cpg = CmsWebCore.Common.AppLib.getContent(pg.PageID, Request.QueryString["lang"], true);

    System.Xml.XmlNodeList videos = (_cpg.ContentXml == null) ? null : _cpg.ContentXml.SelectNodes("//Widget[@name='Videos']/Widget[@name='Video']");
    Dictionary<String, String> videoDic = new Dictionary<String, String>();
    foreach (System.Xml.XmlNode video in videos)
    {
        System.Xml.XmlNode title = video.SelectSingleNode("Field[@name='Title']");
        String titleStr = title == null ? string.Empty : title.InnerText;
        videoDic.Add(video.Attributes["id"].Value, titleStr);
    }
}
<tr class="datafield">
    <td class="label" style="vertical-align:top;">@fpm["flabel"]: </td>
    <td class="field @fpm["ftype"]" fid="@fpm["fname"]">
        <select id="@fpm["fname"]" name="@fpm["fname"]" class="spgsel">
            <option value="">----Please Select----</option>
            @foreach (string id in videoDic.Keys) {
                string sel = (videoId == id) ?" selected":"";
                <option value="@(id)" @sel>@(videoDic[id])</option>
            }
        </select>
    </td>
</tr>