	...
	Map<String, Map<String, List<String>>> formMap = new HashMap<String, Map<String, List<String>>>() {
		{
			put("en", new HashMap<String, List<String>>() {
				{
					put("Form1",
							Arrays.asList("Minimal Form Interface<br/>Simplistic, single input view form"));
					put("QuestionFormOptions",
							Arrays.asList("What's your favorite movie?", "Where do you live?",
									"What time do you got to work?", "How do you like your veggies?",
									"What book inspires you?", "What's your profession?",
									"What's your profession?"));
					put("SendAnswers", Arrays.asList("Send answers"));
					put("Form2", Arrays.asList(
							"Text Input Effects<br/>Simple ideas for enhancing text input interactions"));
					put("Form2Text1", Arrays.asList("Hello, I am"));
					put("Form2Text2", Arrays.asList("Username"));
					put("Form2Text3", Arrays.asList("and my email is"));
					put("Form2Text4", Arrays.asList("Website"));
					...
					put("Form2Text10",
							Arrays.asList("For example: <em>Hong Kong Island</em> or <em>Kowloon</em>"));
			...
			put("tc", new HashMap<String, List<String>>() {
				{
					...
					put("QuestionFormOptions", Arrays.asList("你最喜歡哪部電郵?", "你居住在何處?", "你的工作時間是什麼?", "你如何看待素食主義者",
							"什麼書最能激勵你?", "你最擅長那個方面?"));
					...
					put("Form2Text4", Arrays.asList("網址"));
			...
			put("sc", new HashMap<String, List<String>>() {
				{
					...
					put("QuestionFormOptions", Arrays.asList("你最喜欢哪部电邮?", "你居住在何处?", "你的工作时间是什麽?", "你如何看待素食主义者",
							"什麽书最能激励你?", "你最擅长那个方面?"));
					...
					put("Form2Text4", Arrays.asList("网址"));
					put("Form2Text5", Arrays.asList("我想吃"));
			...
			<section class="pos text-effect">
				<div class="form2-style"><%=formMap.get(lang).get("Form2Text1").get(0)%>
					<span class="input input--ruri"> <input
						class="input__field input__field--ruri" type="text" id="input-26" />
					...
					<span class="input input--ruri"> <input
						class="input__field input__field--ruri" type="text" id="input-27" />
				...
				<div id="nl-form" class="nl-form">
					<span><%=formMap.get(lang).get("Form2Text5").get(0)%> </span> <select>
						<%
							for (int i = 0; i < formMap.get(lang).get("Form2Option1").size(); i++) {
						%>
						<option value="<%=i + 1%>" <%=i == 0 ? "selected" : ""%>><%=formMap.get(lang).get("Form2Option1").get(i)%></option>
						<%
							}
						%>
					...
					<select>
						...
						%>
						<option value="<%=i + 1%>" <%=i == 0 ? "selected" : ""%>><%=formMap.get(lang).get("Form2Option2").get(i)%></option>
						<%
							}
						%>
					</select>
					...
					<select>
						...
						%>
						<option value="<%=i + 1%>" <%=i == 0 ? "selected" : ""%>><%=formMap.get(lang).get("Form2Option3").get(i)%></option>
						<%
							}
						%>
					</select>
